...
with tempfile.TemporaryDirectory() as temp:
    ...
    with patch.multiple(sync,GLOBAL_RULES=rules,STATE=home/'rules-state.json',STATUS=home/'sync-status.json',BACKUP=home/'backup.md'), patch.object(sync,'load_config',return_value=('https://example.invalid',token)):
        with patch.object(sync,'verify_token'),patch.object(sync,'urlopen',side_effect=error(410,b'{"error":"client_rule_distribution_retired"}')),contextlib.redirect_stdout(io.StringIO()) as out:
            sync.main()
        assert json.loads(sync.STATUS.read_text())['status']=='distribution_disabled'
        assert '没有下载或更新规则' in out.getvalue()
        assert rules.read_text()=='Other unrelated rules.\n'
        ...
        assert json.loads(sync.STATUS.read_text())['status']=='check_failed'
        release={'version':1,'created_at':'2026-09-22','rules':[{'category':'用户操作','rule':'缺少商品资料时请先补充。'}]}
        ...
        assert rules.read_text().startswith('Other unrelated rules.')
...
print('Learning checks: exact auth response, rejection, retired-download status, no false protection claim, signed rules and unrelated content preservation passed')
